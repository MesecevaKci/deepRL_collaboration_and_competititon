...
class MADDPG:
    ...
    def __init__(self, random_seed, num_agents, state_size, action_size):
        self.agents = [DDPGAgent(state_size,action_size,random_seed) for agent in range(num_agents)]
        # create shared replay buffer
        self.sharedMemory = ReplayBuffer(BUFFER_SIZE, BATCH_SIZE, num_agents)
    ...
    def save_weights(self):
        for index, agent in enumerate(self.agents):
            torch.save(agent.actor_local.state_dict(), 'agent{}_checkpoint_actor_final.pth'.format(index+1))
            torch.save(agent.critic_local.state_dict(), 'agent{}_checkpoint_critic_final.pth'.format(index+1))
...
class DDPGAgent():
    ...
    def step(self, sharedMemory):
        if len(sharedMemory) > BATCH_SIZE:
            experiences = sharedMemory.sample()
    ...
    def learn(self, experiences, gamma):
        ...
        """
        #states, actions, and next_states are per individual agent (i.e. it is a list)
        ...
        next_actions = [self.actor_target(state) for state in states]        
